...
实现与实际数据验收见 [推送与报价审计](strategy-push-data-audit-2026-09-05.md)。

- 方向锁例外只影响对应候选；selector 和 delivery 共享规则。赢家路径否决后继续
  审查候选，最多三个，未检查者明确标记预算耗尽。
- 报告复用 Core 最终决策；交易记录冻结管理合同、策略常量及内容/依赖哈希。
  实际入队前重取动作时钟；过期观点拒绝，过期报价必须刷新。账户持仓为 UNKNOWN。
- 日历改为既有 Worker 刷新与 Core 只读；按 FF/Fed 独立保存覆盖、last-good 与失败。
  任一 pre-event 阻断优先，空 seed 和部分/过期覆盖不能证明无事件。
- ATM 跨式证据使用实际两腿源时间、provider、strike；每五秒源时间桶最多一份观测。
  更换 ATM/provider 重置局部扩张证据，保留整段展示高低；跨 strike 不直接计算
  同一结构的 15 分钟衰减。处理次数不再充当市场观察数。
- 收益标签显式区分完成、报价缺口、截断、会话错误与不支持的合同；默认 EV 表
  不能混入其他管理政策。旧错误路径版本继续标记待重算。
- 送达后回放按首个新鲜 BBO 核对原限价，并从假设入场时刻计算时间退出；行情满足
  限价仍不代表实际成交。负组合平仓价值必须按现金成本记账，机器舍入不得漏掉止损边界。
  收敛蝶管理升为 `hold_1555.v2`，明确关闭追踪启动，删除用 1000% 阈值近似关闭的做法；
  旧记录保持其历史 v1 合同。

版本为 `strategy_policy.bootstrap.v66`。本次不修改既有入场价差/贷记阈值或 TP/SL
比例；当铁鹰 3C 止损超过翼宽时明确展示其几何限制。仍为人工候选、前向未验证，
无自动下单权限。未新增服务、数据库、队列、依赖或 Rust contract。
