...
def _directional_lane(
    *,
    ...
    if evidence.get("edge_status") == "observed_local_skew_edge":
        score += 1
        contributions.append(
            {
    ...
    available = any(row.get("status") == "available" for row in relevant)
    if available:
        status = "observed"
    else:
        status = "unavailable"
    ...
    return {
        ...
        "edge_status": evidence.get("edge_status") or "unknown",
