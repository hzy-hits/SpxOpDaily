...
10. v46 起 RTH 铁鹰开放当日第一个合格人工候选；v52 基线只在 10:00–11:00 ET 枚举，Put/Call 分别从 Schwab 最新 Greeks 中选择绝对 Delta 不超过且最接近 20Δ 的短腿，两侧固定 10 点翼，并锁定当天首个满足贷记、风险与几何门的行权价。exact 四腿 BBO age ≤15 秒/source skew ≤2 秒，Greeks age ≤15 秒，定义风险 ≤$1,000，普通 `VOL_CONTRACTION_BALANCE` 保守贷记占翼宽 25%–55%。v53 另允许最近 20 分钟内从 `RISK_EXPANSION` 因果转为 `EXPANSION_TO_CONTRACTION` 的平衡路径使用 23%–55% 贷记，但 Put/Call 较小一侧须贡献总贷记至少 25%；其余时间、Delta、翼宽、新鲜度与风险门不变。ATM IV、左右 25Δ skew、联合曲面归因以及 GTH/RTH ATM 跨式/ATM IV 会话高低只作风险说明和非正向上下文；高值或缺失不得拒绝铁鹰、锁死当日资格或绕过其他 hard gate。回购价 ≤0.5C 止盈、≥3C 止损（净亏 200%）、15:45 ET 硬退；每 RTH session 最多一张，GTH 继续 map-only，`automatic_ordering=false`。该合同仍为 `forward_unvalidated_user_override`，不得声明长期 alpha。
11. v48 起 GTH 已确认水平/回收证据可通过分钟级人工门禁，不再要求已推广 first-touch 模型：当前 1m 必须同向，5m 反向幅度不得超过 0.5×ATR5m；仍要求上游因果证据与有效期、exact BBO、借记/翼宽 ≤45%、定义风险 ≤$1,000，并保留方向锁、冷却、人工-only 与 `automatic_ordering=false`。2026-09-01 起用户明确授权欧洲时段 Asia High/Low 突破接受、外侧回踩失败并重新延续作为同一门禁的结构源；单次越界、重新收回区间或除第 20 条 v61 特例外的纯趋势切换仍不可授权。`GTH_WIDTH_SCAN`、`GTH_DELTA_SCAN` 和其他纯趋势切换背景继续不可授权。
...
14. v55 起已授权 RTH Directional Debit Vertical 不再要求 `rth_environment=RISK_EXPANSION`，`direction_valid_but_entry_too_late` 也不再作为 hard gate；两者只保留为环境/入场质量说明。核心环境输入缺失仍失效关闭，并保留宏观、PIN、ATR、目标/行权价路径、exact BBO、setup 专属证据和每组 `$1,000` 定义风险上限。IC/Butterfly 环境合同与 `automatic_ordering=false` 不变。
15. v56 起取消已授权 RTH Directional Debit Vertical 的绝对美元风险 hard gate；候选仍必须是合法定义风险结构并展示最大亏损，由人工执行者决定张数和账户风险预算。IC、Butterfly、GTH 各自已有的风险合同以及 `automatic_ordering=false` 不变。
16. v57 起，已通过因果 ES 放量、1m/5m 同向、PIN、ATR、目标空间、行权价路径与 exact BBO hard gate 的 `ES_VOLUME_MOMENTUM`，在生产仅缺少可选 `strategy_edge_model.v1.json` 时可进入 5 分钟人工候选并标记 `forward_unvalidated_user_override`；存在模型时仍服从 promoted/domain/正 edge 结果，无效、陈旧或未推广模型不回退。RTH breakout 在 inside-to-outside crossing、20 秒接受与 ES 同向之后，可由回踩再突破或额外 10 秒持续扩展确认，继续复用 `RTH_LEVEL_CONFIRMATION` 的固定 15 点 Vertical 与原有人工门；到期且完整离开原水平后才允许新 generation 回踩再入。`fade`、`automatic_ordering=false`、IC/Butterfly/GTH 合同不变。
17. v58 起，RTH 用完整 ES 一分钟路径因果识别 ONH/ONL、OR15H/OR15L 的 Sweep/Reclaim、MSS proxy 与 Displacement，但只拥有现有 Directional Debit Vertical 的非正向排序降权：仅 `MSS_DISPLACEMENT_CONFIRMED` 参与，同向为 0，反向最多 `-0.05`。它不得新增 setup、产生/翻转方向、绕过 hard gate 或单独创建 Trade Ready；缺失、过期、仅 Sweep/MSS 对旧决策零影响，FVG 不进入生产。候选继续人工-only、`automatic_ordering=false`。
18. v59 起，RTH 已授权 Directional Debit Vertical 在 VIX1D、ATM IV、ATM 跨式输入可用而仅缺 `breadth_above_vwap` 时，将广度降级为人读提醒，不再触发 `rth_environment_inputs_unavailable`；环境不得因此获得方向权限或正向加分。铁鹰、STABLE_PIN 蝶式以及任一其他核心环境输入缺失继续失效关闭，其他 hard gate 与 `automatic_ordering=false` 不变。
19. v60 起，`ES_VOLUME_MOMENTUM` 在本周期刚发生 break reclaim 时不授权；当前 1m/5m 方向若逆着至少 `0.5×ATR5m` 的 15m 冲动，必须有 15 分钟内同向新破位。其赢家历史路径至少 20 个 session、shadow choice 为 NO_TRADE、亏损概率至少 75% 且 P90 仍不高于零时失效关闭。Pin 主动通知仅允许已满足 3 快照/10 分钟确认的稳定中轴；形成中中心只留 Desk View。人工候选卡固定展示 ICT 参考，但 ICT 仍不得产生、翻转或授权方向。`automatic_ordering=false` 不变。
20. v61 起，Europe segment 内由现有 Globex trend machine 两次因果确认的 `gth_es_trend_transition` 可作为 `EUROPE_TREND_TRANSITION` 进入 v48 分钟人工门；事后趋势腿起点只作说明，不冒充信号时间。Asia、US premarket、无分段或静态 trend 背景仍不可授权。该 setup 继续要求 5 分钟 TTL、当前 1m 同向、5m 反向不超过 `0.5×ATR5m`、IBKR exact BBO、借记/翼宽 ≤45%、定义风险 ≤$1,000、方向锁/冷却，并标记 `forward_unvalidated_user_override`、`automatic_ordering=false`。
21. v62 起，唯一赢家附加同钟历史路径后，标记 `forward_unvalidated_user_override` 且 risk objective 覆盖至少 20 个独立 session 的候选，在 P90 净 PnL ≤0，或 risk objective <0 且亏损概率 ≥70% 时失效关闭；该门覆盖前向未验证 Debit Vertical/Butterfly，不再只限 ES 动量。`ES_VOLUME_MOMENTUM` 若候选方向上的 VWAP 距离与 15m 冲动同时超过 `2×ATR5m`，必须存在 15 分钟内同向新破位，否则只观察不发人工卡。样本不足/不可用路径不冒充负证据，promoted model 权限不变。人工候选卡固定展示 Gamma 代理风险：正代理提示容易压回并建议等待新破位/重新接受，负代理只说明已确认方向可能放大，过渡区只说明反馈切换；该说明无方向、确认、否决或加分权限，缺失不阻断，dealer sign 仍标记 unknown。`automatic_ordering=false` 不变。
...
32. v69（S1/S3，2026-09-11）：按用户要求取消 RTH 铁鹰每日一张、10:00–11:00 窗和整日首组锁腿。RTH 每次决策重新按当时 Schwab exact BBO/Greeks 选原 20Δ/10 点翼，入场时间为合法 RTH 且 09:30≤ET<15:45；15:45 管理退出、25%/转换23%贷记、环境及其他经济门不变。候选入队同步取消 RTH IC 专属每日上限和通用方向次数上限，保留15分钟冷却及机会幂等；不推断实际持仓。GTH 配额、锁腿与合同不变。策略版本及 RTH 证据 hash 同步更新，仍 forward-unvalidated/manual-only。Bark transport、资金流/墙位通知权限未改。蝶式按第33条滚动未来60分钟合同执行。证据见 docs/rth-entry-contract-v69.md。
