...
- **订阅结构**：基础合约（SPX/SPY/ES/MES）常驻；GTH 与正常交易日美东 15:30–17:00 次日预采窗口使用 84 条 SPXW 预算，其中 56 条 ATM/关键位热区常驻、28 条在 sampling planner 的 rolling groups 上逐片轮换。SPY options 由 Schwab 承担，IBKR 不再为其保留行数；容量跟踪器遇到 ticker-limit 会自动降容。
