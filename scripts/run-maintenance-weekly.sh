...
report_json="$(uv run --no-sync spx ops maintenance dry-run --json --no-write)"
...
case "$action_level" in
  prune|critical_stop_raw)
    ...
    uv run --no-sync spx ops maintenance prune --execute
    ;;
  *)
    echo "[maintenance-weekly] usage below prune threshold; audit-only prune"
    uv run --no-sync spx ops maintenance prune
    ;;
esac
...
uv run --no-sync spx ops maintenance trim-review-audit
