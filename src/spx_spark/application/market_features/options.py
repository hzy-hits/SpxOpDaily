...
def update_atm_straddle_session(
    ...
    *,
    ...
    The durable state stores only extrema and the latest observation.  The
    projection adds current-vs-GTH comparisons for Desk Map and strategy
    audit; it does not grant entry authority.
    """
...
def _atm_straddle_session_projection(
    ...
    *,
    ...
    for session_segment in ("gth", "rth"):
        bucket = state.get(session_segment)
        bucket = bucket if isinstance(bucket, dict) else {}
        projection[session_segment] = {
            "observations": int(bucket.get("observations") or 0),
            "straddle_mid": _extrema_projection(
                bucket,
                "straddle_mid",
                current=current_straddle,
            ),
            "atm_iv": _extrema_projection(
                bucket,
                "atm_iv",
