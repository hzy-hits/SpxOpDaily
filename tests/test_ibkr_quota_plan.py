...
def test_validation_allocation_keeps_twenty_four_line_reserve() -> None:
    ...
    assert plan.reserve_lines == 24


def test_fallback_allocation_expands_spxw_and_keeps_ten_line_reserve() -> None:
    plan = plan_ibkr_option_allocation(fallback=True)
    assert plan.mode is IbkrQuotaMode.FALLBACK
    assert (plan.hot_option_lines, plan.rotation_option_lines) == (54, 24)
    assert plan.option_lines == 78
    assert plan.reserve_lines == 10
