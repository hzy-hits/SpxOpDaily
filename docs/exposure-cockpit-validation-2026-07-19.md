...
- Live accumulator/API：`spx-spark-surface-live.service`，Unix socket `published/spxw-surface/runtime/live/live-api.sock`
...
Live kernel 的部署前生产规模探针（2026-07-17 数据、5 点价格网格）为：front 159 contracts × 67 future buckets 约 2.865 s，next 80 × 67 约 1.503 s。该结果低于 Live API 15 s fail-closed timeout，但不是周一真实 RTH 的延迟验收；真实 publisher → accumulator → browser 的 p95/p99 必须在首个部署后交易日另行记录。
...
Live 与 Replay 现已共用同一固定 matrix contract。Live accumulator 只能从部署后第一个已验证 RTH 快照开始累积；盘中首次启动之前的列永久标记 Missing，不能从 later snapshot 回填。2026-07-19 是周日，因此生产只能验收 healthy/waiting 与 surface 503 的 fail-closed 路径；首个真实交易日的端到端列积累仍需在 RTH 观察后才能宣称 operational acceptance。
