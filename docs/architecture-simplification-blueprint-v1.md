...
状态：**历史设计基线；当前施工范围以执行方案第 0 节的 2026-08-08 收口决定为准。**
...
配套执行文档：`docs/architecture-simplification-execution-plan-v1.md`（对本方案的事实核实、偏差澄清与逐阶段任务卡）。两份文档冲突时，以执行文档为准，因为它锚定了仓库真实文件。P5-2 剩余 helper 已接受为技术债，P6 Rust 退出和 P7 数据平台重写均已延期；三者不得被重新解释为当前必做队列。
...
### 9.1 历史推荐：退出 Rust 生产控制面（当前延期）

2026-08-08 收口后，本节只保留为未来重新立项时的设计依据。当前 Rust
system units 继续作为冻结的生产 owner 运行，只修生产故障；没有验证过的 Python
等价链路、独立维护窗口和明确重新授权时，不执行 P6 cutover 或删除。
