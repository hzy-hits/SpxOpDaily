...
| 2026-08-08 | production safety: delivery priority/restart | +0 / -0 | production +22 / -5 | +0 / -0 | +0 / -0 | +0 / -0 | +0 / -0 | 单线程 Huey 中最终 Bark/飞书/operator 投递使用原生 priority 10，慢 LLM `alert_pipeline` review 使用 -10；现有安全恢复函数接入 worker startup，pre-transport 中断可重排、可能已发送的仍 uncertain；不增 worker/queue/table；31 tests、Ruff、Import Linter、Huey task priority smoke PASS；部署前实测通知虽 delivered 但曾排队约 99 秒 |
