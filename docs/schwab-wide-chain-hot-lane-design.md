...
class SchwabCadenceSettings:
    ...
    normal_hot_quote_seconds: float = 2.0
    active_hot_quote_seconds: float = 1.5
    burst_hot_quote_seconds: float = 1.5
...
class SchwabWideChainSettings:
    ...
    target_expected_move_multiple: float = 2.5
    target_min_width_points: float = 150.0
    target_min_usable_strikes: int = 40
    target_min_two_sided_ratio: float = 0.80
    max_gap_multiple: float = 2.0
    recenter_drift_points: float = 10.0

@dataclass(frozen=True)
class SchwabHotLaneSettings:
    atm_pair_count: int = 32
    boundary_neighbor_pairs: int = 2
    wall_neighbor_pairs: int = 2
    minimum_dynamic_symbol_reserve: int = 10
    rebuild_min_interval_seconds: float = 5.0
...
每一次 `session.get()` 之前先登记 attempt，之后补齐 outcome。网络异常也必须留下
observation。rolling usage 以 attempt observation 计数，而不是以成功 snapshot 计数。
...
```text
required_width = max(2.0 * expected_move_points, 100.0)
actual_lower_width
actual_upper_width
usable_strikes
two_sided_ratio
...
4. 选择最近三个有效样本中满足 coverage 的最小 candidate。
5. 连续 10 个样本显示更小 candidate 仍充分，才允许降级，避免 80/100 抖动。
6. 120 仍不满足时结果必须 DEGRADED/BLOCKED，不继续无界扩大。
...
| QQQ + IWM chains | 30s | 30s | 60s | 4 / 4 / 2 |
| **合计** | | | | **64 / 78 / 82** |

planned ceiling 固定为 84/min，即使用 nominal 120 的 70%，严格保留 36 attempts/min
（30%）供 retry、OAuth、operator probes 和短时调度碰撞。BURST 仍留 2 attempts/min
的 planner 内部余量，gateway 以 120/min 做最终串行限速。
...
10. NORMAL/ACTIVE/BURST 预算分别不超过 64/78/82 nominal plan。
...
本设计将 NORMAL/ACTIVE/BURST 计划提高到约 64/78/82 attempts/min，接近 84 的
