...
- **订阅结构**：IBKR 仅保留 SPX/ES 基础锚点与 SPXW；VIX 家族、ETF、MES 和跨指数上下文由 Schwab 承担，IBKR slow poll 默认关闭。GTH 与正常交易日美东 15:30–17:00 次日预采窗口使用 84 条 SPXW 预算，其中 56 条 ATM/关键位热区常驻、28 条在 sampling planner 的 rolling groups 上逐片轮换；exact-leg pin 从 rotation 抢占，不增加 86 行峰值。容量跟踪器遇到 ticker-limit 会自动降容。
...
| 类别 | 文件 | 内容 |
| --- | --- | --- |
| CFD 标的 | `config.py` | `IBKR_VERIFY_CFDS` 为显式 opt-in，生产默认关闭 |
