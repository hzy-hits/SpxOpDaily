...
from spx_spark.features.exposure_surface import SurfaceGridConfig
...
class ReplayCatalog:
    ...
    def _provider_paths_between(
        ...
        *,
        ...
        """Resolve only UTC hour partitions overlapping one causal window."""

        lower = as_utc(start)
        upper = as_utc(end)
        if upper <= lower or provider not in {"ibkr", "schwab"}:
            return ()
        paths: list[Path] = []
        cursor_date = lower.date()
        while cursor_date <= upper.date():
            provider_root = (
                self.quote_lake_root
                / f"date={cursor_date.isoformat()}"
                / f"provider={provider}"
            )
            for candidate in provider_root.glob("hour=*/quotes.parquet"):
                try:
                    resolved = candidate.resolve(strict=True)
                    hour = int(resolved.parent.name.removeprefix("hour="))
                ...
                if partition_start < upper and partition_end > lower:
                    paths.append(resolved)
            cursor_date += timedelta(days=1)
        return tuple(sorted(set(paths)))

    def _surface_source_paths(self, session_date: date) -> tuple[Path, ...]:
        """Bind v2 cache/provenance to basis, GTH, and RTH mixed providers."""

        window = session_surface_window(session_date)
        paths = {
            *self._provider_paths_between(
                "schwab",
                start=window.previous_rth_open,
                end=window.previous_rth_end,
            ),
            *self._provider_paths_between(
                "schwab",
                start=window.session_start,
                end=window.session_end,
            ),
            *self._provider_paths_between(
                "ibkr",
                start=window.session_start,
    ...
    def _verified_source_hashes(
        ...
        *,
        ...
        """Return content hashes cached only for an unchanged stat fingerprint."""

        if self._source_fingerprint(paths) != source_fingerprint:
            raise ReplayCacheError("replay_cache_source_context_changed")
        with self._source_hash_lock:
            cached = self._source_hash_memory.get(source_fingerprint)
            if cached is not None:
                self._source_hash_memory.move_to_end(source_fingerprint)
                return dict(cached)
        ...
        with self._source_hash_lock:
            self._source_hash_memory[source_fingerprint] = dict(verified)
            self._source_hash_memory.move_to_end(source_fingerprint)
            while len(self._source_hash_memory) > MAX_VERIFIED_SOURCE_HASH_CONTEXTS:
                self._source_hash_memory.popitem(last=False)
        return verified
    ...
    def _scan_viable_frames(
        ...
        *,
        ...
        latest_candidate_at = session_close - timedelta(
            seconds=SurfaceGridConfig().min_tau_seconds
        )
        query = """
    ...
    def session_surface(
        ...
        *,
        ...
        frames = self.viable_frames(session_date)
        legacy_source_paths = self._relevant_paths(session)
        legacy_source_fingerprint = self._source_fingerprint(legacy_source_paths)
        surface_source_paths = self._surface_source_paths(session_date)
        if not surface_source_paths:
            raise ReplayRequestError(
                "replay_session_surface_source_unavailable",
                status=HTTPStatus.NOT_FOUND,
            )
        surface_source_fingerprint = self._source_fingerprint(surface_source_paths)
        context = TrendContext(
            data_root=self.data_root,
            session_date=session_date,
            open_at=surface_window.session_start,
            close_at=surface_window.session_end,
            close_grace_elapsed_at=self._close_grace_elapsed_at(session),
            close_grace_policy=SESSION_CLOSE_GRACE_POLICY,
            close_grace_seconds=SESSION_CLOSE_GRACE_SECONDS,
            frame_minutes=self.frame_minutes,
            ...
            projection_policy=self.projection_policy,
            projection_policy_sha256=self.projection_policy_sha256,
            source_paths=surface_source_paths,
            source_fingerprint=surface_source_fingerprint,
            frames=frames,
        )

        def current_source_fingerprint() -> str:
            self.get_session(session_date)
            return self._source_fingerprint(self._surface_source_paths(session_date))

        verified_surface_hashes: dict[str, str] | None = None
        verified_legacy_hashes: dict[str, str] | None = None
        ...
        def session_frame_loader(frame_at: datetime) -> dict[str, object]:
            ...
            return self._read_cached_frame(
                cache_path,
                requested=frame_at,
                source_paths=legacy_source_paths,
                source_fingerprint=legacy_source_fingerprint,
                verified_source_hashes=verified_legacy_hashes,
            )
        ...
        try:
            verified_surface_hashes = self._verified_source_hashes(
                surface_source_paths,
                source_fingerprint=surface_source_fingerprint,
            )
            verified_legacy_hashes = self._verified_source_hashes(
                legacy_source_paths,
                source_fingerprint=legacy_source_fingerprint,
            )
            return materialize_session_surface(
                ...
                verified_source_hashes=verified_surface_hashes,
            )
    ...
    def sessions_payload(self) -> dict[str, object]:
        return build_sessions_payload(self)
