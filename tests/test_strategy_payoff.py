...
def test_globex_hmm_publishes_cross_state_not_path() -> None:
    ...
    assert regime["policy_version"] == "strategy_policy.bootstrap.v64"
...
def test_close_convergence_produces_one_manual_butterfly_without_pin_authority(
    ...
    assert candidate["authorization_policy"] == "strategy_policy.bootstrap.v64"
...
def test_failed_break_does_not_select_call_vertical_past_target_or_remaining_move() -> None:
    ...
    assert decision["decision_type"] == "CALL_DEBIT_VERTICAL", decision["why_not"]
    assert decision["candidate"]["setup_kind"] == "ES_VOLUME_MOMENTUM"
    assert decision["candidate"]["long"]["strike"] == 7755.0
    assert decision["candidate"]["short"]["strike"] == 7760.0
...
def test_gth_fact_pack_prefers_causal_gth_atr_when_rth_atr_is_missing() -> None:
    ...
    lineage["atr"] = {"value": None}
    lineage["gth_atr"] = {"value": 2.25}

    facts = build_market_fact_pack(payload, _state(now), now)

    assert facts["session"]["mode"] == "gth"
    assert facts["path"]["atr_5m"] == pytest.approx(2.25)
    assert facts["capabilities"]["path"]["atr_ready"] is True
