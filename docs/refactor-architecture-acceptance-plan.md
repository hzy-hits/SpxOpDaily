...
- `$SPX` chain 每 5 秒一次、当前从 `strikeCount=80` 起按覆盖自适应；
...
```text
46 SPXW core option lines
+ 38 SPXW rotating option lines
+ 4 direct anchors
+ 6 temporary/slow-poll lines
= 94 concurrent lines
```

VIX 家族、ETF、MES 和跨指数上下文由 Schwab 承担，IBKR slow poll 与 SPY option lane
默认关闭；exact-leg pin 抢占 rotation 行而不提高峰值。6 条余量留给取消延迟、重规划与
