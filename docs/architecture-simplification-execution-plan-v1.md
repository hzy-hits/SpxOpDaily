...
| 2026-08-08 | production safety: strategy-distribution Rust contract parity | +0 / -0 | Rust production +30 / -0 | +0 / -0 | +0 / -0 | +0 / -0 | contract +17 / -2 | 生产 bridge active 但因 Python v1 已输出 `n_raw`、`n_effective`、`historical_sessions` 而拒绝整个 probability lane；Rust typed contract 现显式接纳并校验这三项，保留 `deny_unknown_fields`、`action_authority=none` 与自动下单关闭。domain 7 tests、bridge 6 tests、fmt、diff-check PASS；仅作为冻结 Rust 的生产故障修复，不恢复 P6 扩建 |
