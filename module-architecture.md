...
L3 analytics       options_map, iv_surface, market_context, human_focus, position_events, strategy/*
L2 providers       ibkr/*, schwab/*, hyperliquid/*, polymarket/*, mock_collector
L1 infrastructure  config, storage, state_io, sampling, runtime_mode, provider_adapter
L0 foundation      marketdata, market_calendar, alert_model
```

分层规则(守护测试强制执行):

1. 任何模块只能 import 同层或更低层的模块。
2. L0 模块不得 import 任何 spx_spark 内部模块(彼此之间也不行,保持零依赖)。
...
### L0 foundation(零内部依赖)
- `market_calendar.py` — stdlib-only 的美股交易日、09:30-16:00/13:00 提前
  收盘、17:00 ET research rollover 与 post-close review 日期解析。
- `marketdata.py` — 纯领域模型:`InstrumentId/Quote/OptionGreeks/Provider/
  MarketDataQuality/QuoteFreshness/QuoteUseDecision/ProviderState` + 通用归一化工具
  (`clean_float/parse_timestamp/classify_quote_quality/quote_use_decision/as_utc/
  elapsed_ms/bool_or_none` 等)。
...
- `config.py` — 全部 Settings dataclass + `from_env()`;公共 env 读取工具
  ...
  import,禁止自己写 `os.getenv` 解析布尔/浮点);兼容入口
  `default_spxw_expiry` 委托给 L0 `market_calendar`。
- `storage.py` — JSONL 写入、`LatestState/LatestStateStore`。
- `state_io.py` — mode `0600` 的原子 JSON 替换与跨进程 advisory lock。
...
- `options_map.py`(OI/gamma/wall)、`iv_surface.py`、`market_context.py`、
  `human_focus.py`(人类关注上下文,含 vol_context)、`position_events.py`
  (持久 position outbox 与逐事件确认)、`strategy/micopedia.py`。

### L4 alerting
- `alert_profile.py` — 时段窗口定义。
- `alert_engine.py` — 告警评估(从 L3/L1 读取,产出 `Alert`,调用 notifier)。
- `position_alerts.py` — SPXW 持仓快照转 observation、pending event 渲染与确认对账。
- `notifier/` — 通知管道包(见 §4A)。
...
```python
LAYERS = {
    "config": 1, "marketdata": 0, "market_calendar": 0, "alert_model": 0,
    "storage": 1, "state_io": 1, "sampling": 1, "runtime_mode": 1, "provider_adapter": 1,
    "ibkr": 2, "schwab": 2, "hyperliquid": 2, "polymarket": 2, "mock_collector": 2,
    "options_map": 3, "iv_surface": 3, "market_context": 3,
    "human_focus": 3, "position_events": 3, "strategy": 3,
