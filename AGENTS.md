...
2. 策略候选第一版固定五类（NO_TRADE、Call/Put Debit Vertical、Call/Put Butterfly）；v4 仅新增 setup `EVENT_SETTLEMENT_THRESHOLD`，仍复用 Call/Put Debit Vertical，不新增 payoff 类型。RTH 方向人读卡的 setup 是 `ES_VOLUME_MOMENTUM`（ES 量比 + 1m/5m 动量）；`TREND_PULLBACK` / `FAILED_BREAK_RECLAIM` 不再授权 RTH 人读卡。扩大候选空间需用户批准。
