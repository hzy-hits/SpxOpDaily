    ...
    pub fn write_desk_map(
        ...
        let research_input = match (&projection.research_context, projection.session) {
            ...
            (None, MarketSession::Gth) => {
                "research_context_status=gth_not_applicable\nGTH desk maps are a live 5-20Δ short / 10-wide iron-condor and width-scan card, not a health heartbeat and not a trade ticket. Do not mention HMM, cash index, close-location, session high/low, bootstrap research, that research is unavailable, 可看, 当前无有效机会, 当前无人工操作, or an unpassed Call/Put debit spread as the action. Say 本图不是下单卡; actionable direction is only on 「SPX 人工候选」."
                    .to_owned()
            }
            (None, _) => format!(
