...
from spx_spark.application.order_map.payload_readiness import (
    apply_gth_em_usage as _apply_gth_em_usage,
    has_open_position_risk as _has_open_position_risk,
    payload_has_retryable_candidate_gap as _payload_has_retryable_candidate_gap,
    payload_is_thin as _payload_is_thin,
    recent_market_frame_es as _recent_market_frame_es,
)
...
def build_order_payload_with_retry(
    ...
    *,
    ...
    if state is not None:
        ...
        payload["experimental_research_signals"] = load_json(
            Path(storage_settings.data_root) / "latest" / "experimental_research_signals.json"
        )
