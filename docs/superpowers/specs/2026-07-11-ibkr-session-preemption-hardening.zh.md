...
- 形态 C（10197）：CONFLICT_WAIT → persist unavailable + purge + `defer_market_data_after_conflict(60s)`（`stream_collector.py:2184-2205`；`runtime_policy.ibkr_conflict_probe_seconds=60`，`runtime.yaml:703-705`）。defer 期间 `market_data_allowed()` 为 False（`stream_collector.py:692-694`）；若 `account_read_enabled=false`（当前生产值，`runtime.yaml:1143-1145`），`connection_required()` 也为 False → 外层循环 persist unavailable + purge（`stream_collector.py:2013-2020`）。**10197 分支现状是干净的。**
