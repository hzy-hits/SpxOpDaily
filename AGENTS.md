...
10. v46 起 RTH 铁鹰开放当日第一个合格人工候选；v50 当前合同只在 10:00–11:00 ET 枚举，Put/Call 分别从 Schwab 最新 Greeks 中选择绝对 Delta 不超过且最接近 20Δ 的短腿，两侧固定 10 点翼，并锁定当天首个合格行权价。exact 四腿 BBO age ≤15 秒/source skew ≤2 秒，Greeks age ≤15 秒，定义风险 ≤$1,000。v49 的其余门保持：保守贷记占翼宽 25%–55%，入场 ATM IV ≤0.2374713681、左右 25Δ skew 平均值 ≤0.0313827831；三项曲面数据缺失时失效关闭。曲面判定冻结在当天首个满足贷记、风险与几何门的候选上；未通过则该 RTH 整日不再开铁鹰，不得等待 IV 回落后重新放行。回购价 ≤0.5C 止盈、≥3C 止损（净亏 200%）、15:45 ET 硬退；每 RTH session 最多一张，GTH 继续 map-only，`automatic_ordering=false`。该门仍为 `forward_unvalidated_user_override`，不得声明长期 alpha。
