    ...
    pub fn write_desk_map(
        ...
        let research_input = match (&projection.research_context, projection.session) {
            ...
            (None, MarketSession::Gth) => {
                "research_context_status=gth_not_applicable\nGTH desk maps are a live 5-25Δ short / 10-wide iron-condor and width-scan card, not a health heartbeat. Do not mention HMM, cash index, close-location, session high/low, bootstrap research, that research is unavailable, 可看, or an unpassed Call/Put debit spread as the action."
                    .to_owned()
            }
            (None, _) => format!(
